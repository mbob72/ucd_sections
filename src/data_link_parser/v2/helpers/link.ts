...
const readLinkPart = function* (params: DataLinkParserInterfaces.v2.Params, data: Record<string, any>): Generator<any, any, any> {
    ...
    for (current of dataLink) {
        ...
        current = dataLink.getCurrentValue();
        if (current[2] === '<' || current[2] === '$' || current[2] === '(' || current[2] === '\\') continue;
        if (
            current[2] === '/' ||
    ...
    if (!isObject(data) && !Array.isArray(data)) return defaultValue; // todo: is it right?
    if (part && /^\d+$/.test(part) && Array.isArray(data)) return data.length > Number(part) ? data[Number(part)] : defaultValue;
    if (part && isObject(data)) return hasOwnProperty(data, part) ? data[part] : defaultValue;
    return defaultValue;
};
/**
