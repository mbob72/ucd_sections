...
export const sectionsDidMountProcessor = (processor) => {
    return (sections, setState) => {
        ...
        if (typeof setState !== 'function')
            throw new Error('setState must be a function.');
        if (!Array.isArray(sections))
            throw new Error('Sections list must be an array.');
        const promises = [];
        const newSections = [];
        for (let index = 0; index < sections.length; index++) {
            if (!strictlyIsObject(sections[index]))
...
export const generatorComputation2 = () => {
    return function* (input, env) {
        ...
        return new Promise((resolve, reject) => {
            setTimeout(
                () => resolve({ ...input, value: value + '_value2' }),
                10000
            );
        });
    };
