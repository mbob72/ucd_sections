...
export const log = (marker = null, additional = null) => {
    return (input, env) => {
        if (marker && typeof marker !== 'string') throw new Error('[error] log computation: marker must be a string.')
        console.warn(`[debug] ${marker || 'log computation'}:`, { input, env, additional })
        return input
    }
}
        ...
        if (Array.isArray(action)) {
            ...
            return action.reduce((acc, func) => {
                if (typeof func !== 'function') {
                    return acc.then(() => {
                        throw new Error(`[error] runAction: array mus contain a list of functions. The type '${typeof func} is given.'`)
                    })
                }
...
export const applyFilters = (...filters) => {
    return (input) => {
        ...
        for (const filter of filters) {
            if (typeof filter !== 'function') throw Error('[error] applyFilters: filter must be a function.')
            value = filter(value)
        }
        return { ...input, value }
    }
...
export const debounce = (computation, time) => {
    if (typeof computation !== 'function') throw new Error('[error] debounce: The computation must be a function.')
    time = Number(time)
    if (Number.isNaN(time)) throw new Error('[error] debounce: The time must be a number.')
    return (input, env) => {
        return new Promise((resolve, reject) => {
            const { currentSchemaObject: { _objectId_: key } } = env
            if (!Number.isInteger(key)) throw new Error('[error] debounce: The key must be a number.')
...
export const delay = (timeOut = 0) => {
    ...
    if (Number.isNaN(timeOut)) throw new Error('[error] Delay: delay value must be a number.')
    return (input, env) => {
        return new Promise((resolve) => setTimeout(() => resolve(input), timeOut))
    }
}
