...
import { branch, compose, renderNothing, withPropsOnChange, withStateHandlers } from 'recompose'
...
const EVENTS = { _onBlur_: 'onBlur', _onChange_: 'onChange', _onClick_: 'onClick' }

const getSetStateParams = (valueLink, value, schema, actions, after, context, match, location) => {
...
const SectionField = compose(
    ...
    withPropsOnChange(['context', 'parsedSchema'], ({ setLoadingState, context, parsedSchema, setState, match, location, fieldComponents, computations }) => {
        const {
            _objectId_,
            _value_ = '',
            _visible_ = true,
        ...
        if (!Number.isInteger(_objectId_)) throw new Error('[error] SectionField: _objectId_ must be a number.')
        let { _type_ = '' } = parsedSchema
        if (!Array.isArray(after)) throw new Error('[error] SectionField: _after_ must be an array.')
        const visible = typeof _visible_ === 'string'
            ? simpleDataParser({ dataLink: _visible_, data: context, renderFunctions: computations })
            : !!_visible_
        if (isDataLink(_type_)) {
            _type_ = dataLinkParser({ dataLink: getDataLink(_type_), data: context, renderFunctions: computations })
        ...
        if (!Comp) throw new Error('[error] SectionField: DefaultField must be defined.')
        ...
        for (const eventType of Object.getOwnPropertyNames(_handlers_)) {
            ...
            handlers[EVENTS[eventType]] = e => setState(
                getSetStateParams(
                    _value_,
                    e.target.value,
                    parsedSchema,
...
)(({ parsedSchema, level, context, handlers, errors, Comp, computations, loading, styles }) => {
    return (
        <Comp
            styles={styles}
            loading={loading}
...
export default ({ parsedSchema }) => {
    return <SectionsContext.Consumer>
        {({ computations, sectionComponents, fieldComponents, context, setState, styles }) => {
            return <SectionField
                styles={styles}
                context={context}
                parsedSchema={parsedSchema}
