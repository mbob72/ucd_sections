...
const SectionEntry = compose(
    ...
    branch(
        ({ context = null, parsedSchema = null }) => !(
            (context && strictlyIsObject(context) && parsedSchema && strictlyIsObject(parsedSchema))
        ),
        renderNothing
    ),
    withProps(({ history, context, computations, parsedSchema, sectionComponents, level }) => {
        ...
        if (!Array.isArray(_sections_) || !Array.isArray(_fields_)) throw new Error('Sections: _sections_/_fields_ must be an array.')
        ...
        const Comp = _type_ && _type_ in sectionComponents
...
)(({ parsedSchema, level, tokenParams, context, Comp, sections, fields, setState, computations, styles }) => {
    return (
        <Comp
            styles={styles}
            context={context}
